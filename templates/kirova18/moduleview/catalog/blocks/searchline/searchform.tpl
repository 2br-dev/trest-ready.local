...
<form method="GET" class="query on" action="{$router->getUrl('catalog-front-listproducts', [])}" {if !$param.hideAutoComplete}id="queryBox"{/if}>
    {hook name="catalog-block-searchform:form" title="{t}Строка поиска: форма{/t}"}
        <input type="text" class="theme-form_search{if !$param.hideAutoComplete} rs-autocomplete{/if}" name="query" value="{$query}" autocomplete="off" data-source-url="{$router->getUrl('catalog-block-searchline', ['sldo' => 'ajaxSearchItems', _block_id => $_block_id])}" placeholder="{t}Поиск по каталогу{/t}">
